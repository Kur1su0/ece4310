...
for (r=3; r<ROWS-3; r++)
  ...
  for (c=3; c<COLS-3; c++)
    ...
    var=sqrt(var)/49.0;
    if (var < 1.0)	/* XXX changed 1.0 condition for seeding a new region is low var */
      {
...
while (qt != qh)
  {
  if ((*count)%10 == 0)	/* recalculate average after each 50 pixels join */
    {
    average=total/(*count); 
    // printf("new avg=%d\n",average);
  ...
  for (r2=-1; r2<=1; r2++)
    for (c2=-1; c2<=1; c2++)
      ...
      if (abs((int)(image[(queue[qt]/COLS+r2)*COLS+queue[qt]%COLS+c2])
		-average) > 10){ //XXX change 10 ->50
        
       // printf("diff: %d\n",abs((int)(image[(queue[qt]/COLS+r2)*COLS+queue[qt]%COLS+c2])
        //        -average));
        continue;
       }
      //Centroid.
      p_r = queue[qt]/COLS + r2;
      p_c = queue[qt]%COLS + c2;
      distance = sqrt(SQR( abs(centroid_r-p_r) )+ SQR( abs(centroid_c-p_c) ) );
  //    printf("dis %d\n",distance);  
          if( distance > 10){
              //printf("dis %d\n",distance);         
              continue;
